...
- Name: ui\mixed.spec.ts >> Mixed UI + API tests >> verificar usuario por API y luego hacer login por UI
- Location: tests\ui\mixed.spec.ts:7:7
...
Call log:
  - waiting for getByTestId('add-to-cart-undefined')
...
> 6  |     await this.page.getByTestId(`add-to-cart-${productName}`).click();
  ...
  8  | 
  9  |   async gotoCart() {
  10 |     await this.page.getByTestId("shopping-cart-link").click();
  11 |   }
  12 | 
  13 |   async filterProducts() {
  14 |     await this.page.getByTestId("product-sort-container").selectOption("lohi");
  15 |   }
  16 | }
  17 | 
```