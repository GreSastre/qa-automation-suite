...
- Name: ui\saucedemo.spec.ts >> Flujo de compra en Saucedemo >> logout
- Location: tests\ui\saucedemo.spec.ts:115:7
...
Call log:
  - waiting for getByTestId('logout-sidebar-link')
...
```ts
  ...
  2 | export class MenuPage {
  3 |   constructor(private page: Page) {}
  4 |   async logOut() {
  5 |     await this.page.getByRole("button", { name: "Open Menu" }).click();
> 6 |     await this.page.getByTestId("logout-sidebar-link").click();
    |                                                        ^ Error: locator.click: Test timeout of 30000ms exceeded.
  7 |   }
  8 | }
  9 | 
