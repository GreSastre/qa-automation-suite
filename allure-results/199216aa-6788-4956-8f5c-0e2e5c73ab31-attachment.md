...
- Name: ui\saucedemo.spec.ts >> Flujo de compra en Saucedemo >> agregar dos productos al carrito
...
Locator: locator('.inventory_item_name').filter({ hasText: 'Sauce Labs Backpak' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.inventory_item_name').filter({ hasText: 'Sauce Labs Backpak' })
...
```ts
  ...
  89  |   });
  90  | 
  91  |   test("agregar dos productos al carrito", async ({ authenticatedPage }) => {
  92  |     const inventoryPage = new InventoryPage(authenticatedPage);
  93  |     await inventoryPage.addProduct("sauce-labs-backpack");
  94  |     await inventoryPage.addProduct("sauce-labs-bolt-t-shirt");
  95  |     await expect(authenticatedPage.locator(".shopping_cart_badge")).toHaveText(
  96  |       "2",
  97  |     );
  98  |     const productoMochila = authenticatedPage
  99  |       .locator(".inventory_item_name")
  100 |       .filter({ hasText: "Sauce Labs Backpak" });
  101 |     const productoCamiseta = authenticatedPage
  102 |       .locator(".inventory_item_name")
  103 |       .filter({ hasText: "Sauce Labs Bolt T-shirt" });
> 104 |     await expect(productoMochila).toBeVisible();
      |                                   ^ Error: expect(locator).toBeVisible() failed
  105 |     await expect(productoCamiseta).toBeVisible();
  106 |   });
  107 | 
  108 |   test("redireccion correcta despues del login", async ({
  109 |     authenticatedPage,
  110 |   }) => {
  111 |     await authenticatedPage.waitForURL(/inventory/);
  112 |     await expect(authenticatedPage.getByText("Products")).toBeVisible();
  113 |   });
  114 | 
  115 |   test("logout", async ({ authenticatedPage }) => {
  116 |     const menuPage = new MenuPage(authenticatedPage);
  117 |     await menuPage.logOut();
  118 |     await expect(authenticatedPage).toHaveURL(/saucedemo/);
  119 |     await expect(authenticatedPage.getByText("Swag Labs")).toBeVisible();
  120 |   });
  121 | 
  122 |   test("filter", async ({ authenticatedPage }) => {
  123 |     const inventoryPage = new InventoryPage(authenticatedPage);
  124 |     await inventoryPage.filterProducts();
  125 |     const producto = authenticatedPage
  126 |       .locator(".inventory_item_name")
  127 |       .filter({ hasText: "Sauce Labs Onesie" });
  128 |     await expect(producto).toBeVisible();
  129 |   });
  130 | });
  131 | 
```