...
- Name: ui\saucedemo.spec.ts >> Flujo de compra en Saucedemo >> Pedido Exitso
- Location: tests\ui\saucedemo.spec.ts:65:7

# Error details

```
TypeError: InventoryPage.addProduct is not a function
```
...
```ts
  ...
  2  | export class InventoryPage {
  3  |   constructor(private page: Page) {}
  4  | 
  5  |   async addProduct(productName: string) {
  6  |     await this.page.getByTestId(`add-to-cart-${productName}`).click();
> 7  |     await InventoryPage.addProduct("sauce-labs-backpack");
     |                         ^ TypeError: InventoryPage.addProduct is not a function
  8  |   }
  9  | 
  10 |   async gotoCart() {
  11 |     await this.page.getByTestId("shopping-cart-link").click();
  12 |   }
  13 | 
  14 |   async filterProducts() {
  15 |     await this.page.getByTestId("product-sort-container").selectOption("lohi");
  16 |   }
  17 | }
  18 | 
```